...
const MAX_MESSAGE_SIZE: usize = 10 * 1024 * 1024; // ← ДОБАВИТЬ ЭТУ СТРОКУ!
...
const MINING_BATCH_SIZE: u64 = 5000000;

// ДОПОЛНИТЕЛЬНЫЕ P2P КОНСТАНТЫ
const SYNC_BATCH_SIZE: u64 = 500;
const SYNC_MAX_RETRIES: u32 = 3;
const SYNC_RETRY_DELAY_SECS: u64 = 10;
const MAX_BLOCKS_IN_RESPONSE: u32 = 500;
const BLOCK_REQUEST_TIMEOUT_SECS: u64 = 30;
...
// ✅ НОВАЯ СТРУКТУРА ДЛЯ СОХРАНЕНИЯ КОШЕЛЬКА В ФАЙЛ
            ...
            P2PMessage::Blocks(blocks) => {
                ...
                match verification_result {
                    ...
                    Err(e) => {
                        ...
                        // Можно забанить пира за отправку невалидных блоков
    ...
    .and_then(move |tx_hex: String| {
        ...
        async move {
            ...
            match bincode::deserialize::<Transaction>(&tx_data) {
                Ok(tx) => {
                    // Сначала вычисляем txid
                    let txid = tx.txid(&mut node.argon2);
                    
                    // Клонируем для broadcast ДО перемещения
                    let tx_clone = tx.clone();
                    
                    // ✅ ИСПОЛЬЗУЕМ НОВЫЙ МЕТОД add_transaction_to_mempool
